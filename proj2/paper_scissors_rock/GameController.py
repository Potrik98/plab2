...
class GameController:
    ...
    def _process_results(self, results: [int]) -> ([[float]], [[float]]):
        ...
        return ([total_score, player1_wr],
                [[-a for a in total_score], player2_wr],)

    def _plot_results(self,
                      player1_results: [[float]],
                      player2_results: [[float]]) -> None:
        assert len(player1_results) == len(player2_results)
        labels = ["Score", "Win rate"]
        h = []
        for r1, r2, i in zip(player1_results,
                             player2_results,
                             range(len(player1_results))):
